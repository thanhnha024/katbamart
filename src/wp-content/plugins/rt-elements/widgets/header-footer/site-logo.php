 ...
 * @since 1.3.0
 */
class RTS_Site_Logo extends Widget_Base {
	...
	public function get_title() {
		return __( 'Site Logo', 'rtelements' );
	}
	...
	protected function register_content_site_logo_controls() {
		...
		$this->add_control(
			'site_logo_fallback',
			[
				'label'       => __( 'Custom Image', 'rtelements' ),
				'type'        => Controls_Manager::SWITCHER,
				'yes'         => __( 'Yes', 'rtelements' ),
				'no'          => __( 'No', 'rtelements' ),
				'default'     => 'no',
		...
		$this->add_responsive_control(
			...
			[
				...
				'default'            => 'center',
				'selectors'          => [
					'{{WRAPPER}} .hfe-site-logo' => 'text-align: {{VALUE}};',
				],
		...
		$this->add_control(
			'caption',
			[
				'label'       => __( 'Custom Caption', 'rtelements' ),
				'type'        => Controls_Manager::TEXT,
				'default'     => '',
				'placeholder' => __( 'Enter caption', 'rtelements' ),
				'condition'   => [
					'caption_source' => 'yes',
				],
				'dynamic'     => [
		...
		$this->add_control(
			'link_to',
			[
				'label'   => __( 'Link', 'rtelements' ),
				'type'    => Controls_Manager::SELECT,
				'default' => 'default',
				'options' => [
					'default' => __( 'Default', 'rtelements' ),
					'none'    => __( 'None', 'rtelements' ),
					'file'    => __( 'Media File', 'rtelements' ),
					'custom'  => __( 'Custom URL', 'rtelements' ),
				],
			]
		);
		...
		$this->add_control(
			...
			[
				...
				'placeholder' => __( 'https://your-link.com', 'rtelements' ),
				'condition'   => [
					'link_to' => 'custom',
				],
				'show_label'  => false,
		...
		$this->start_controls_section(
			...
			[
				'label' => __( 'Sticky Logo', 'rtelements' ),
			]
		);

		$this->add_control(
			'stikcy_logo_fallback',
			[
				'label'       => __( 'Sticky Image', 'rtelements' ),
				'type'        => Controls_Manager::SWITCHER,
				'yes'         => __( 'Yes', 'rtelements' ),
				'no'          => __( 'No', 'rtelements' ),
				'default'     => 'no',
	...
	protected function register_site_logo_styling_controls() {
		...
		$this->add_responsive_control(
			'width',
			[
				'label'              => __( 'Logo Width', 'rtelements' ),
		...
		$this->add_control(
			...
			[
				'label'     => __( 'Transition Duration', 'rtelements' ),
				'type'      => Controls_Manager::SLIDER,
				'range'     => [
					'px' => [
						'max'  => 3,
		...
		$this->add_control(
			'hover_animation',
			[
				'label' => __( 'Hover Animation', 'rtelements' ),
	...
	protected function render() {
		...
		if ( ! empty( $settings['custom_image_sticky']['url'] ) ) {
			$image_data2 = wp_get_attachment_image_src( $settings['custom_image_sticky']['id'], $image_size, true );
			$hide_desktop_logo =  'hide_logo_desktop';
			...
			<div class="hfe-site-logo-set">           
				<div class="hfe-site-logo-container">
					<?php if(!empty($image_url)): ?>
						<img class="hfe-site-logo-img <?php echo esc_attr( $hide_desktop_logo ); ?>"  src="<?php echo esc_url( $image_url ); ?>" alt="<?php echo esc_attr( Control_Media::get_image_alt( $settings['custom_image'] ) ); ?>"/>
					<?php endif; ?>
					<?php if(!empty($image_url2)): ?>
					<img class="hfe-site-sticky-logo-img "  src="<?php echo esc_url( $image_url2 ); ?>" alt="<?php echo esc_attr( Control_Media::get_image_alt( $settings['custom_image_sticky'] ) ); ?>"/>
				<?php endif; ?>
				</div>
			</div>
		<?php if ( $link ) : ?>
