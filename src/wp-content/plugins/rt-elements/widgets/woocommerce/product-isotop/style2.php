<?php $img_size = $settings['thumbnail_size'];?>
<div data-rating="<?php echo $arating; ?>" data-sell="<?php echo $price; ?>" class="element-item col-xxl-<?php echo esc_html($settings['product_columns']);?> col-xl-3 col-lg-4 col-sm-6 col-xs-1 grid-item <?php echo $filter_class;?>">
		...
		<?php if(has_post_thumbnail()): ?>
			...
			?>
				<div class="sale--box">
					...
					if ($is_new){
						?> <span class="new"><?php if($settings['new_text']){
							echo esc_html($settings['new_text']);
						}else{
							esc_html_e( 'NEW', 'rtelemenets' );
						}
						?></span>  <?php
					}
					if( $is_feat ){
