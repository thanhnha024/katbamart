...
while($best_wp->have_posts()): $best_wp->the_post();
	...
	<div class="courses-item e-dash-slider-2">
            	    ...
            	    <?php if(($settings['cate_show_hide'] == 'yes') ){ ?>
            	    	<div class="cats">         		
            	    		<?php $cat = get_the_terms( $post_id , $taxonomy );?>
            			 	<ul class="course-categories">                          
            	                <?php 
								$term_css = $term_color = $term_bg = '';
            	                foreach ($cat as $catin ) { 
									$term_bg    = get_term_meta($catin->term_id, 'course_color', true );
									$term_color = get_term_meta($catin->term_id, 'cat_color', true ); 

									if ( !empty($term_color) ) {    
									    $term_css .= "color: {$term_color};";  
									}

									if ( !empty($term_bg) ) { 
									    $term_css .= "background: {$term_bg};";  
									}
            	                ?>

            	                <li><a style="<?php echo $term_css; ?>" href="<?php echo get_category_link($catin->term_id);?>"><?php echo $catin->name ;?></a></li> 
            	                <?php   } ?>
            	          	</ul>
            	    	</div>
            	    <?php } ?>
        ...
        <div class="content-part">   
            ...
            <div class="bottom-part">	
                <div class="course-price"> <span class="price"><?php  echo esc_html($price_text); ?></span> </div>
